      ...
      <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-md w-full">
        ...
        <div className="mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-2xl font-bold text-gray-800">{showAdminLogin ? 'Admin Login' : 'Welcome Back!'}</h2>
            <button
              type="button"
              onClick={() => setShowAdminLogin(!showAdminLogin)}
              className="text-sm text-gray-500 hover:text-gray-700 underline"
            >
              {showAdminLogin ? 'User Login' : 'Admin'}
            </button>
          </div>
          <p className="text-gray-600">{showAdminLogin ? 'Admin access only' : 'Enter your credentials to continue'}</p>
        </div>
